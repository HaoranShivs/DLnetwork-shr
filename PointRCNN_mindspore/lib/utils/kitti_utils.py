...
def rotate_pc_along_y_mindspore(pc, rot_angle):
    """
    :param pc: (N, 512, 3 + C)
    :param rot_angle: (N)
    :return:
    TODO: merge with rotate_pc_along_y_mindspore in bbox_transform.py
    """
    cosa = mindspore.ops.Cos(rot_angle).view(-1, 1)  # (N, 1)
    sina = mindspore.ops.Sin(rot_angle).view(-1, 1)  # (N, 1)

    raw_1 = mindspore.ops.Concat([cosa, -sina], dim=1)  # (N, 2)
    raw_2 = mindspore.ops.Concat([sina, cosa], dim=1)  # (N, 2)
    R = mindspore.ops.Concat((raw_1.unsqueeze(dim=1), raw_2.unsqueeze(dim=1)), dim=1)  # (N, 2, 2)

    pc_temp = pc[:, :, [0, 2]]  # (N, 512, 2)

    pc[:, :, [0, 2]] = mindspore.nn.MatMul(pc_temp, R.permute(0, 2, 1))  # (N, 512, 2)
