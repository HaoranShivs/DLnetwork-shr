...
import os
import pickle
from mindspore.tensor import from_numpy
...
class KittiRCNNDataset(KittiDataset):
    ...
    def get_rpn_features(rpn_feature_dir, idx):
        ...
        if cfg.RCNN.USE_SEG_SCORE:
            ...
            rpn_seg_score = Sigmoid(from_numpy(rpn_seg_score)).numpy()
        else:
    ...
    def apply_gt_aug_to_one_scene(self, sample_id, pts_rect, pts_intensity, all_gt_boxes3d):
        ...
        while try_times > 0:
            ...
            boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(
                from_numpy(pts_rect), from_numpy(enlarged_box3d.reshape(1, 7)))
    ...
    def get_rcnn_sample_info(self, roi_info):
        ...
        boxes_pts_mask_list = roipool3d_utils.pts_in_boxes3d_cpu(from_numpy(rpn_xyz),
                                                                 from_numpy(pooled_boxes3d))
        pt_mask_flag = (boxes_pts_mask_list[0].numpy() == 1)
        ...
        if aug_pts.shape[0] == 0:
            ...
            if cfg.RCNN.USE_MASK:
                if cfg.RCNN.MASK_TYPE == 'seg':
                    ...
                    pts_mask = roipool3d_utils.pts_in_boxes3d_cpu(from_numpy(aug_pts),
                                                                  from_numpy(aug_roi_box3d.reshape(1, 7)))
    ...
    def canonical_transform_batch(pts_input, roi_boxes3d, gt_boxes3d):
        ...
        # rotate to the direction of head
        gt_boxes3d_ct = kitti_utils.rotate_pc_along_y_mindspore(from_numpy(gt_boxes3d_ct.reshape(-1, 1, 7)),
                                                            from_numpy(roi_ry)).numpy().reshape(-1, 7)
        gt_boxes3d_ct[:, 6] = gt_boxes3d_ct[:, 6] - roi_ry
        pts_input = kitti_utils.rotate_pc_along_y_mindspore(from_numpy(pts_input), from_numpy(roi_ry)).numpy()
    ...
    def get_rcnn_training_sample_batch(self, index):
        ...
        if fg_num_rois > 0 and bg_num_rois > 0:
            ...
            rand_num = from_numpy(rand_num).type_as(gt_boxes3d).long()
